...
///ZYLHJ///
Field3D Grad_par_phi,Grad_par_Pe,GradPhi2, Dperp2Phi, bracketPhiP, Dperp2Pi,Btemp,bracketphiNi,bracketphiVi,bracketPiVi,Ni_curvature1,Ni_curvature2,bracketphiTe,bracketphiTi,bracketphipei,Te_curvature1,Te_curvature2,Te_curvature3,Te_current1,Ti_curvature1,Ti_curvature2,Ti_curvature3,Ti_current1;
...
Field3D Grad_par_pei,Grad_par_pn,Grad_par_Jpar;
...
// Load 2D profiles
  ...
  OPTION(options, term_parallel_current, true);
//ZYLHJ
...
Grad_par_Jpar.setLocation(CELL_CENTRE);
Grad_par_Jpar.setBoundary("Tn");
...
Grad_par_phi.setLocation(CELL_CENTRE);
Grad_par_phi.setBoundary("Tn");

Grad_par_Pe.setLocation(CELL_CENTRE);
Grad_par_Pe.setBoundary("psi");
...
{
   Grad_par_Jpar= Grad_par(Jpar);
   Grad_par_Jpar.applyBoundary();
   mesh->communicate(Grad_par_Jpar);


    Tepara1 = bmag*tbar/(ee*Ni_x*density_unit*Lbar*Lbar*MU0);
//    Tepara2 = Bmag*Bmag*Me
Te_current1 = 0.66667*0.71*temp_Te*Grad_par_Jpar;
...
if(term_parallel_current)
{
   Grad_par_phi= Grad_par(phi);
   Grad_par_phi.applyBoundary();
   mesh->communicate(Grad_par_phi);


   Tipara1 = bmag*bmag/(Ni_x*density_unit*KB*Te_x*eV_K*MU0);
   Ti_current1 = 0.66667*Jpar*Grad_par_phi/temp_Ni;
...
Jpara3 = 0.71*KB*Te_x*eV_K*tbar/(ee*bmag*Lbar*Lbar);
   ...
   Grad_par_Pe= Grad_par(Pe);
   Grad_par_Jpar.applyBoundary();
   mesh->communicate(Grad_par_Pe);
/*
Dperp2psi = Delp2(psi);
Dperp2psi.applyBoundary();
mesh->communicate(Dperp2psi);
*/
ddt(psi) = -Jpara0*Grad_par_phi/B0 - eta*Jpar;   //+ Jpara1*eta*Dperp2psi;
ddt(psi) += Jpara2*Grad_par_Pe/(temp_Ni*B0);
