...
///ZYLHJ///
Field3D Grad_par_phi,Grad_par_te,Grad_par_Pe;
...
Grad_par_phi.setBoundary("phi");

Grad_par_te.setLocation(CELL_CENTRE);
Grad_par_te.setBoundary("Te");

Grad_par_Pe.setLocation(CELL_CENTRE);
Grad_par_Pe.setBoundary("pei");
 	...
 	for (int jz=0;jz<mesh->ngz;jz++)
		...
		if(initial_PF_edgeval)
			...
			Ni[jx][jy][jz] = Ni_edge;
                        Ti[jx][jy][jz] = Ti_edge;
                        Te[jx][jy][jz] = Te_edge;
//                          psi[jx][jy][jz] = minimum_val;
//                          jpar[jx][jy][jz] = minimum_val;
//                          phi[jx][jy][jz] = minimum_val;
//                          V[jx][jy][jz] = minimum_val;
                       }
                   }
...
Te_current1 = 0.66667*0.71*temp_Te*Grad_par_LtoC(Jpar);
...
if(term_parallel_current)
{
   ...
   Ti_current1 = 0.66667*Jpar*Grad_par_LtoC(phi)/temp_Ni;
...
//************************************
  ...
  //output.write("Now updata Vi \n");
   Grad_par_pei=Grad_par_LtoC(pei);
...
//Grad_par_phi = Grad_par_LtoC(phi);
Grad_par_phi.applyBoundary();
mesh->communicate(Grad_par_phi);

Grad_par_Pe = Grad_par(Pe);
Grad_par_Pe.applyBoundary();
mesh->communicate(Grad_par_Pe);

Grad_par_te = Grad_par(temp_Te);
Grad_par_te.applyBoundary();
mesh->communicate(Grad_par_te);

Dperp2psi = Delp2(psi);
Dperp2psi.applyBoundary();
mesh->communicate(Dperp2psi);

ddt(psi) = -Jpara0*Grad_par_phi/B0 - eta*Jpar;
ddt(psi) += Jpara2*Grad_par_Pe/(temp_Ni*B0);
ddt(psi) += Jpara3*Grad_par_te/B0;
