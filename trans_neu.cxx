...
Field2D psixy;
// 3D evolving fields
Field3D  Te, Ni, Vi, Ti, Nn,Tn, Vn, Nm ,Vmx;             // Vi and Vn are paralell velocity
...
// Load 2D profiles
  ...
  OPTION(options, initial_profile_linear, true); // priority III
  ...
  OPTION(options, terms_energy_interchange, true);  
 ...
 if(!include_curvature)
  ...
  if(profiles_lasttimestep)
	...
	ifstream pFile1,pFile2,pFile3,pFile4,pFile5,pFile6,pFile7;
...
//************************************
  ...
  ddt(Te) =  0.6667*kappa_Te*Grad2_par2(Te)/temp_Ni
   ...
   if(terms_energy_interchange) ddt(Te)  -= 2.*fmei*(Te-Ti)/tau_ei;
...
//************************************
  ...
  //ddt(Ti)=0.0;
   ...
   if(terms_energy_interchange) ddt(Ti)  += 2.*fmei*(Te-Ti)/tau_ei;
    ...
    for (int jx=0;jx<mesh->ngx;jx++)
        {
           ...
           for (int jy=0;jy<mesh->ngy;jy++)
	      {
			 ...
			 BoutReal CF_BC_yhalf=0.5*(CF_BC_y0+CF_BC_y1);
                         ...
                         BoutReal CF_BC_zhalf=0.5*(CF_BC_z0+CF_BC_z1);
                         BoutReal CF_zwidth=CF_BC_zhalf-CF_BC_z0;
                         BoutReal CF_exp_Zdecay=exp(-(z_glb-CF_BC_zhalf)*(z_glb-CF_BC_zhalf)/CF_zwidth/CF_zwidth);
                         if(CF_BC_z1>=1.1)CF_exp_Zdecay=1.;   // no decay in Z for 2D SMBI simulations

       	                 result[jx][jy][jz]=value*CF_exp_decay*CF_exp_Zdecay; 
...

{
  ...
  //release memory in case of OOM error
  delete xrup;
  delete xrdn;
